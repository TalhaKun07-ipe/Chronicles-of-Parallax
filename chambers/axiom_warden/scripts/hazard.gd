...
func _ready() -> void:
	...
	match kind:
		...
		"slam": visual = Geo.ring(self, Vector3(0, 0.04, 0), 0.8, 0.08, warning_mat)
	...
	_play_sfx("boss_warning" if kind != "bolt" else "boss_bolts")
...
func intersects(feet: Vector3, height: float, player_radius: float, player_mode: int) -> bool:
	...
	match kind:
		...
		"slam":
			var dist: float = Vector2(feet.x - global_position.x, feet.z - global_position.z).length()
			return dist >= previous_radius - player_radius - 0.25 and dist <= radius + player_radius + 0.25 and low < 0.32 and high > -0.2
		"bolt":
